  ...
  registration(user: UserModel): Observable<AuthResponse> {
    return this.http.post<UserModel>(this.apiUrl, user).pipe(
      map((newUser: UserModel) => {
        const token = this.generateToken(newUser); // Secure token generation
        const authResponse: AuthResponse = {
          token,
          expiresIn: 3600, // Example expiration time in seconds
          issuedAt: new Date(),
          role: newUser.role as 'Admin' | 'HR' | 'Employee',
        };
    ...
    return this.http.get<UserModel[]>(this.apiUrl, { params }).pipe(
      map((users) => {
        ...
        if (user && user.password === credentials.password) {
          const token = this.generateToken(user); // Secure token generation
          const authResponse: AuthResponse = {
            token,
            expiresIn: 3600,
            issuedAt: new Date(),
